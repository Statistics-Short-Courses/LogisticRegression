...
Recall that the *error term* $\epsilon$ in a regression model captures the difference between the observed outcome $Y$ and the *mean*, or [*expected*]{.glossary expected_value} outcome $E[Y]$ from the model. When $Y$ was continuous, $E[Y]$ represented the average value of $Y$ for given values of the predictors. What does $E[Y]$ represent when $Y$ is a binary outcome?
...
The choice of using 0 and 1 is not arbitrary. When we code a binary outcome this way, the mean of the outcome variable, $E[Y]$ has a direct interpretation:
$$
E[Y] = P(Y=1) = p
$$

That is, the expected value (mean) of a binary variable coded as 0/1 is equal to the *probability* of the outcome being 1.
...
```{r}
mean_pass_rate <- mean(student_performance$pass_fail_num)
mean_pass_rate
```
This means that, on average, the probability of a student passing the exam in Johno's dataset is approximately `r round(mean_pass_rate * 100, 2)`%.
:::

::: key-point
Once a binary outcome is coded as 0/1,  modelling the expected value $E[Y]$ is equivalent to modeling the *probability* of the outcome being 1 (e.g., passing the exam).
:::
...
And obtains the following linear regression model:
```{r}
